...
2. Push to the Space with [`scripts/deploy_hf.sh`](../scripts/deploy_hf.sh) — never `git push hf main` directly.
   ...
   the README, drops `eval/` so the benchmark questions and run
   ...
   it. `HF_DEPLOY_DRY_RUN=1 ./scripts/deploy_hf.sh` builds the branch without pushing.
