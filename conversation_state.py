...
class ConversationStateMachine:
    ...
    @staticmethod
    def rewrite(message: str, subject: dict) -> str:
        ...
        replacements = (
            ...
            (r"\bher\b(?=\s+[a-z])", f"{name}'s"),
            (r"\bit\b", name),
            (r"\b(she|he|him|her|they|them)\b", name),
        )
        rewritten = message
    ...
    def resolve(self, message: str, prior_state: Optional[dict], explicit_subjects: list[dict]) -> dict:
        ...
        if prior.get("mode") == "comparing" and len(comparison_candidates) > 1:
            ...
            if collective or comparative_selection:
                ...
                return {
                    ...
                    "rewritten_query": f"Compare {names}: {message.strip()}",
        ...
        if len(explicit) > 1:
            ...
            if active and correction and "compare" not in message.lower():
                ...
                if selected:
                    ...
                    old_name = str(active.get("name", "")).strip()
                    if old_name:
                        previous = re.sub(re.escape(old_name), str(selected.get("name", "")), previous, flags=re.IGNORECASE)
                    selected_name = str(selected.get("name", "")).strip()
                    rewritten = (
                        previous
                        if selected_name and selected_name.lower() in previous.lower()
                        else self.rewrite(previous, selected)
                    )
                    return self._resolved(
                        rewritten, selected,
        ...
        if len(explicit) == 1:
            ...
            if carry_previous:
                ...
                old_subjects = unique_subjects(
                    ([prior["active_subject"]] if prior.get("active_subject") else [])
                    + list(prior.get("candidate_subjects") or [])
                )
                rewritten = previous
                for old_subject in old_subjects:
                    old_name = str(old_subject.get("name", "")).strip()
                    if old_name:
                        rewritten = re.sub(re.escape(old_name), str(subject.get("name", "")), rewritten, flags=re.IGNORECASE)
                subject_name = str(subject.get("name", "")).strip()
                if not (subject_name and subject_name.lower() in rewritten.lower()):
                    rewritten = self.rewrite(rewritten, subject)
                return self._resolved(
                    rewritten, subject, str(prior.get("last_intent") or intent),
                    used_context=True, prior=prior,
                )
            name = str(subject.get("name", "")).strip()
            canonical_is_explicit = bool(name and name.lower() in message.lower())
            rewritten = message if canonical_is_explicit else self.rewrite(message, subject)
        ...
        if active_scope and not prior.get("active_subject") and not prior.get("candidate_subjects"):
            ...
            rewritten = message
            if scope_name:
                rewritten = re.sub(r"\b(on\s+)?it\b", scope_name, rewritten, flags=re.IGNORECASE)
                if rewritten == message:
                    scope_context = scope_name + (f" about {filter_text}" if filter_text else "")
                    rewritten = f"Regarding {scope_context}, {message.strip()}"
            state = normalize_state(prior)
            state["last_intent"] = intent
            return {
                "resolved": False, "needs_clarification": False,
