...
class OfflineChatbot(RetrievalChatbot):
    ...
    def __init__(self):
        ...
        def llm_callable(prompt):
            self.gemini_calls += 1
            if "standalone retrieval question" in prompt:
                anchor_match = re.search(r"Anchor subject: (.+)", prompt)
                question_match = re.search(r"User question:\n(.+?)\n\nReturn valid JSON", prompt, re.DOTALL)
                anchor = anchor_match.group(1).strip() if anchor_match else ""
                question = question_match.group(1).strip() if question_match else ""
                return json.dumps({"rewritten_query": f"{anchor}: {question}"})
...
class ChatbotRegressionTests(unittest.TestCase):
    ...
    def test_llm_answer_route_controls_entity_registry_gate(self):
        self.bot.entity_registry = [{"section_name": "Tim Cronin", "entity_type": "person"}]
        self.assertTrue(self.bot.should_use_entity_registry(
            "Who is Tim Cronin?",
            {"answer_route": "registry", "question_type": "people_lookup"},
        ))
        self.assertFalse(self.bot.should_use_entity_registry(
            "What research does Tim Cronin do?",
            {"answer_route": "retrieval", "question_type": "specific_fact"},
        ))

    def test_document_neighbor_expansion_stays_inside_source_and_unit(self):
        self.bot.config = type("Config", (), {
            "top_k": 5, "document_neighbor_count": 1, "document_neighbor_limit": 8,
        })()
        self.bot.search_records = [
            {"id": "a-0", "document": "Mary Jane education.", "metadata": {
                "source_path": "Staff.txt", "unit_id": "mary", "chunk_level": "detail", "chunk_index": 0,
            }},
            {"id": "a-1", "document": "Mary Jane research.", "metadata": {
                "source_path": "Staff.txt", "unit_id": "mary", "chunk_level": "detail", "chunk_index": 1,
            }},
            {"id": "b-1", "document": "Another person's research.", "metadata": {
                "source_path": "Staff.txt", "unit_id": "other", "chunk_level": "detail", "chunk_index": 1,
            }},
        ]
        expanded = self.bot.expand_document_neighbors(
            [{"id": "a-0", "document": "Mary Jane education.", "metadata": self.bot.search_records[0]["metadata"], "score": 1.0}],
            "Mary Jane research",
            {},
        )
        self.assertEqual({candidate["id"] for candidate in expanded}, {"a-0", "a-1"})
        self.assertEqual(expanded[1]["retrieval_expansion"] if "retrieval_expansion" in expanded[1] else expanded[1].get("neighbor_of"), "a-0")

    def test_current_queries_prefer_newer_and_undated_sources(self):
        candidates = [
            {"id": "annual-2021", "document": "2021 annual report director", "metadata": {"source_path": "AnnualReport_2021.txt"}, "hybrid_score": 0.8},
            {"id": "annual-2024", "document": "2024 annual report director", "metadata": {"source_path": "AnnualReport_2024.txt"}, "hybrid_score": 0.8},
            {"id": "staff", "document": "Current Executive Director", "metadata": {"source_path": "Staff.txt"}, "hybrid_score": 0.8},
        ]
        ranked = self.bot.rerank_candidates("Who is the current director?", candidates, {})
        ranked = self.bot.apply_freshness_adjustment("Who is the current director?", ranked, {})
        self.assertEqual([candidate["id"] for candidate in ranked], ["staff", "annual-2024", "annual-2021"])

    def test_historical_queries_do_not_apply_freshness_boost(self):
        candidates = [
            {"id": "annual-2021", "document": "2021 annual report director", "metadata": {"source_path": "AnnualReport_2021.txt"}, "hybrid_score": 0.8},
            {"id": "annual-2024", "document": "2024 annual report director", "metadata": {"source_path": "AnnualReport_2024.txt"}, "hybrid_score": 0.8},
        ]
        ranked = self.bot.rerank_candidates("Who was director in 2021?", candidates, {})
        ranked = self.bot.apply_freshness_adjustment("Who was director in 2021?", ranked, {})
        self.assertTrue(all("freshness_boost" not in candidate for candidate in ranked))

    def test_registry_miss_is_not_a_supported_answer(self):
        self.assertFalse(self.bot.has_supported_registry_result({
            "reply": "I found no supported registry evidence for the requested facts.",
            "sources": [],
        }))
        self.assertTrue(self.bot.has_supported_registry_result({
            "reply": "Paul Watanabe's area of expertise is American Foreign Policy. [1]",
            "sources": [{"source_path": "SEED_DOCUMENTS/UniversityAffiliates.txt"}],
        }))

    def test_descriptive_facets_separate_topics_and_audience_from_quantity(self):
        facets = self.bot.detect_requested_fact_facets(
            "What types of topics and participants does it typically bring together?"
        )
        self.assertIn("topic", facets)
        self.assertIn("audience", facets)
        self.assertNotIn("quantity", facets)

    def test_title_and_department_are_retrieval_facets(self):
        facets = self.bot.detect_requested_fact_facets(
    ...
    def test_relationship_prompt_requires_same_evidence_unit(self):
        prompt = self.bot.build_prompt(
            ...
            rewritten_query="What is the East Boston study that VanDeveer's team worked on? (subject: Stacy D. VanDeveer)",
            queried_person="Stacy D. VanDeveer",
            retrieved_context=["Stacy D. VanDeveer worked on the East Boston study."],
            retrieved_metadata=[{"source_path": "AnnualReport2021.txt"}],
        )
        self.assertIn("same evidence unit", prompt)
        self.assertIn("Do not add collaborators or side projects unless the question asks", prompt)

    def test_answer_contract_extracts_count_and_locator_context(self):
        contract = self.bot.build_answer_contract(
            "What are the three program elements of the Climate Careers Curricula Initiative?"
        )
        self.assertEqual(contract["requested_count"], 3)
        self.assertFalse(contract["uses_locator_context"])
        locator_contract = self.bot.build_answer_contract(
            "What multimedia project did the Fifty-Fifty art collective present at the event co-hosted by SSL and the Climigration Network?"
        )
        self.assertTrue(locator_contract["uses_locator_context"])

    def test_answer_contract_detects_count_and_locator_violations(self):
        count_violations = self.bot.validate_answer_contract(
            "What are the three program elements of the Climate Careers Curricula Initiative?",
            "* First [1]\n* Second [1]\n* Third [1]\n* Fourth [1]",
        )
        self.assertTrue(any("exactly 3" in violation for violation in count_violations))
        locator_violations = self.bot.validate_answer_contract(
            "What multimedia project did the Fifty-Fifty art collective present at the event co-hosted by SSL and the Climigration Network?",
            "The project is Portable Memories in Rising Seas [1]. I don't have that information about the event [1].",
        )
        self.assertTrue(locator_violations)

    def test_query_plan_facets_keep_subject_and_stable_ids(self):
        normalized = self.bot.normalize_query_plan(
            {
                "rewritten_query": "Mary Jane education and research",
                "answer_route": "retrieval",
                "answer_requirements": ["education", "research"],
                "facets": [
                    {"question": "What is Mary Jane's education?", "subject": "Mary Jane", "answer_route": "registry"},
                    {"question": "What is Mary Jane's research?", "subject": "Mary Jane", "answer_route": "retrieval"},
                ],
            },
            {"titles": [], "categories": [], "folders": [], "source_paths": [], "entity_names": []},
            "What is her education and research?",
        )
        self.assertEqual([facet["id"] for facet in normalized["facets"]], ["facet_1", "facet_2"])
        self.assertEqual(normalized["facets"][0]["subject"], "Mary Jane")
        self.assertEqual(normalized["facets"][0]["answer_route"], "registry")

    def test_planner_persists_non_registry_subject_and_scope(self):
        normalized = self.bot.normalize_query_plan(
            {
                "rewritten_query": "Who moderated Climate Emergency Preparedness: Art x Cultural Resilience?",
                "subject_decision": {
                    "status": "new_topic",
                    "name": "Climate Emergency Preparedness: Art x Cultural Resilience",
                    "subject_type": "event",
                    "source_scope": {
                        "title": "AnnualReport2021",
                        "source_path": "AnnualReport2021.txt",
                    },
                },
            },
            {"titles": ["AnnualReport2021"], "categories": [], "folders": [], "source_paths": ["AnnualReport2021.txt"], "entity_names": []},
            "Who moderated that event?",
        )
        state = self.bot.build_state_from_query_plan(normalized, [])
        subject = state["active_subject"]
        self.assertEqual(subject["subject_type"], "event")
        self.assertEqual(subject["name"], "Climate Emergency Preparedness: Art x Cultural Resilience")
        self.assertEqual(subject["source_scope"]["title"], "AnnualReport2021")

    def test_planner_keeps_non_registry_candidates(self):
        normalized = self.bot.normalize_query_plan(
            {
                "rewritten_query": "Which subject is this book about?",
                "subject_decision": {"status": "ambiguous"},
                "candidate_subjects": [
                    {"name": "The Great Derangement", "subject_type": "book"},
                    {"name": "Climate Justice Alliance", "subject_type": "organization"},
                ],
        ...
        state = self.bot.build_state_from_query_plan(normalized, [])
        self.assertEqual([subject["subject_type"] for subject in state["candidate_subjects"]], ["book", "organization"])
        self.assertEqual(state["mode"], "awaiting_clarification")

    def test_mixed_plan_adds_registry_facet_as_authoritative_context(self):
        self.bot.entity_registry = [{
            "unit_id": "mary", "title": "Staff", "source_path": "Staff.txt", "source_url": "",
            "section_name": "Mary Jane", "entity_type": "staff_member",
            "detail_text": "Mary Jane studied environmental policy.", "summary_text": "",
        }]
        context, metadata = self.bot.add_registry_facet_context(
            ["retrieval evidence"], [{"source_path": "Projects.txt"}],
            {"facets": [{"id": "facet_1", "subject": "Mary Jane", "question": "What is her education?", "answer_route": "registry"}]},
        )
        self.assertIn("Mary Jane studied environmental policy.", context[-1])
        self.assertEqual(metadata[-1]["retrieval_facet_ids"], "facet_1")

    def test_retrieval_keeps_multi_facet_evidence_in_separate_buckets(self):
        self.bot.collection = type("Collection", (), {"count": lambda self: 1})()
        self.bot.config = type("Config", (), {"top_k": 4, "document_neighbor_limit": 0})()

        candidates_by_query = {
            "Mary Jane education": {
                "id": "education",
                "document": "Mary Jane studied environmental policy.",
            },
            "Mary Jane research": {
                "id": "research",
                "document": "Mary Jane researches climate migration.",
            },
        }

        def retrieve_dense(query, limit, query_route=None):
            candidate = candidates_by_query.get(query)
            if not candidate:
                candidate = {
                    "id": "shared",
                    "document": "Mary Jane works on education and research.",
                }
            return [{
                **candidate,
                "metadata": {"source_path": f"{candidate['id']}.txt"},
                "score": 1.0,
            }]

        self.bot.retrieve_dense_candidates = retrieve_dense
        self.bot.retrieve_bm25_candidates = lambda query, limit, query_route=None: []
        self.bot.fuse_candidates = lambda query_profile, dense_candidates, bm25_candidates: dense_candidates
        self.bot.rerank_candidates = lambda query, candidates, query_profile: candidates
        self.bot.apply_freshness_adjustment = lambda query, candidates, query_profile: candidates
        self.bot.expand_document_neighbors = lambda seeds, query, query_route: seeds

        context, metadata, diagnostics = self.bot.retrieve_context(
            "Mary Jane education and research",
            query_route={
                "answer_requirements": [],
                "facets": [
                    {"id": "facet_education", "question": "Mary Jane education", "answer_route": "retrieval"},
                    {"id": "facet_research", "question": "Mary Jane research", "answer_route": "retrieval"},
                ],
            },
        )

        bucket_documents = {
            metadata_item["retrieval_facet_ids"]: context[index]
            for index, metadata_item in enumerate(metadata)
        }
        self.assertIn("facet_education", bucket_documents)
        self.assertIn("facet_research", bucket_documents)
        self.assertIn("Mary Jane studied environmental policy", bucket_documents["facet_education"])
        self.assertNotIn("Mary Jane studied environmental policy", bucket_documents["facet_research"])
        self.assertIn("Mary Jane researches climate migration", bucket_documents["facet_research"])
        self.assertNotIn("Mary Jane researches climate migration", bucket_documents["facet_education"])
        self.assertEqual(diagnostics["evidence_buckets"]["facet_education"], 1)
        self.assertEqual(diagnostics["evidence_buckets"]["facet_research"], 1)

    def test_adversarial_claim_level_cases_are_not_rejected_yet(self):
        cases = [
            {
                "question": "What is Gabriella Boscio's title?",
                "context": (
                    "Gabriella Boscio is affiliated with the Sustainable Solutions Lab. "
                    "Her child, Sofia Boscio, is a high school student."
                ),
                "bad_answer": "Gabriella Boscio's title is high school student. [1]",
            },
            {
                "question": "What research does Gabriella Boscio do?",
                "context": (
                    "Gabriella Boscio researches coastal resilience. "
                    "Her child Sofia Boscio studies music composition."
                ),
                "bad_answer": "Gabriella Boscio researches music composition. [1]",
            },
            {
                "question": "What is Gabriella Boscio's title and research?",
                "context": (
                    "Gabriella Boscio is a research fellow. "
                    "Her child Sofia Boscio studies music composition."
                ),
                "bad_answer": (
                    "Gabriella Boscio is a research fellow and researches music composition. [1]"
                ),
            },
        ]
        metadata = [{
            "title": "Staff and Affiliates",
            "source_path": "Staff.txt",
            "source_url": "https://example.test/staff",
            "retrieval_facet_ids": "facet_main",
        }]

        for case in cases:
            with self.subTest(question=case["question"]):
                violations = self.bot.validate_answer_contract(case["question"], case["bad_answer"])
                sanitized = self.bot.sanitize_reply_citations(case["bad_answer"], self.bot.extract_sources(metadata))
                self.assertEqual(violations, [])
                self.assertEqual(sanitized, case["bad_answer"])

    def test_multiple_named_subjects_expand_fallback_source_scope(self):
        self.bot.entity_registry = [
            {
                "section_name": "Jen Stevenson Zepeda",
                "entity_type": "board_member",
                "source_path": "SEED_DOCUMENTS/BoardOfDirectors.txt",
                "title": "BoardOfDirectors",
            },
            {
                "section_name": "Johnna Flahive",
                "entity_type": "affiliate",
                "source_path": "SEED_DOCUMENTS/Staff.txt",
                "title": "Staff",
            },
        ]
        route = self.bot.detect_local_query_route(
            "What is Jen Stevenson Zepeda's professional title, and what is Johnna Flahive's research focus?"
        )
        self.assertEqual(route["routing_mode"], "soft")
        self.assertEqual(
            set(route["target_source_paths"]),
            {"SEED_DOCUMENTS/BoardOfDirectors.txt", "SEED_DOCUMENTS/Staff.txt"},
        )
        self.assertIn("multiple named subjects expanded to independent source scopes", route["reason"])

    def test_registry_fact_route_combines_registry_and_facet_sources(self):
        self.bot.entity_registry = [{
            "section_name": "B. R. Balachandran",
            "entity_type": "staff_member",
            "source_path": "SEED_DOCUMENTS/Staff.txt",
            "title": "Staff",
        }]
        self.bot.search_records = [
            {"id": "staff", "document": "B. R. Balachandran is Executive Director.", "metadata": {
                "source_path": "SEED_DOCUMENTS/Staff.txt", "section_name": "B. R. Balachandran",
            }},
            {"id": "project", "document": "B. R. Balachandran's research examines coastal resilience projects.", "metadata": {
                "source_path": "SEED_DOCUMENTS/Projects.txt", "section_name": "Coastal resilience",
            }},
            {"id": "unrelated", "document": "B. R. Balachandran attended a staff meeting.", "metadata": {
                "source_path": "SEED_DOCUMENTS/Annual Reports/AnnualReport2022.txt", "section_name": "Staff meeting",
            }},
        ]
        route = {
        ...
        self.assertTrue(self.bot.should_combine_registry_retrieval(
            "What is B. R. Balachandran's research background?", route
        ))
        expanded = self.bot.expand_registry_candidate_sources(
            "What is B. R. Balachandran's research background?",
            {**route, "combine_registry_retrieval": True, "routing_mode": "hard"},
        )
        self.assertIn("SEED_DOCUMENTS/Staff.txt", expanded["target_source_paths"])
        self.assertIn("SEED_DOCUMENTS/Projects.txt", expanded["target_source_paths"])
        self.assertEqual(expanded["routing_mode"], "soft")

    def test_direct_evidence_recovery_extracts_supported_project_title(self):
        result = self.bot.extract_direct_evidence_answer(
            "What multimedia project did the Fifty-Fifty art collective present at the event co-hosted by SSL and the Climigration Network?",
            [
                "This event, cohosted by the Climigration Network and featuring the Fifty-Fifty art collective's multimedia project Portable Memories in Rising Seas, was an opportunity to reflect on climate change."
            ],
            [{"source_path": "AnnualReport2021.txt"}],
        )
        self.assertEqual(result["reply"], "The project was Portable Memories in Rising Seas. [1]")

    def test_negative_generation_claim_is_removed_when_facet_is_supported(self):
        reply = (
            "Georgia Mavrommati is an Associate Professor at UMass Boston [1]. "
            "Her areas of expertise include complex systems, deliberative valuation, and environmental policy [1]. "
            "The record does not explicitly state her area of research [1]."
        )
        sanitized = self.bot.sanitize_unsupported_negative_claims(
            "What is Georgia Mavrommati's area of expertise at UMass Boston?",
            reply,
            ["Georgia Mavrommati is an Associate Professor. Her areas of expertise include complex systems, deliberative valuation, and environmental policy."],
        )
        self.assertNotIn("does not explicitly state", sanitized)
        self.assertIn("complex systems", sanitized)

    def test_prompt_and_sources_share_evidence_citation_mapping(self):
        metadata = [{
            "title": "AnnualReport2021",
            "source_path": "AnnualReport2021.txt",
            "source_url": "https://example.test/report",
            "retrieval_facet_ids": "facet_event",
            "retrieval_facet_queries": "Who moderated the event?",
        }]
        prompt = self.bot.build_prompt(
            "Who moderated the event?",
            ["Kim Szeto moderated the event."],
            retrieved_metadata=metadata,
        )
        sources = self.bot.extract_sources(metadata)
        self.assertIn("[1] Evidence ID: evidence_1", prompt)
        self.assertIn("Evidence bucket: facet_event", prompt)
        self.assertEqual(sources[0]["evidence_id"], "evidence_1")
        self.assertEqual(sources[0]["evidence_bucket"], "facet_event")

    def test_plan_enrichment_carries_active_subject_into_every_facet(self):
        plan = {
            "rewritten_query": "What awards did she receive?",
            "answer_route": "retrieval",
            "facets": [{"id": "facet_1", "question": "What awards did she receive?", "answer_route": "retrieval"}],
        }
        enriched = self.bot.enrich_query_plan_with_state(
            plan,
            {"active_subject": {"unit_id": "person-johnna", "name": "Johnna Flahive"},
             "active_scope": {"source_path": "Staff.txt"}},
        )
        self.assertIn("Johnna Flahive", enriched["rewritten_query"])
        facet = enriched["facets"][0]
        self.assertEqual(facet["subject_id"], "person-johnna")
        self.assertIn("Johnna Flahive", facet["standalone_query"])
        self.assertEqual(facet["source_scope"]["source_path"], "Staff.txt")
